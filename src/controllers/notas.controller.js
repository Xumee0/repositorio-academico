...
exports.guardarNota = async (req, res) => {
  try {
    if (req.user.rol !== 'docente') {
      return res.status(403).json({ msg: 'Solo docentes pueden calificar' });
    }
    ...
    // Verificar que el proyecto exista y que el curso esté asignado al docente
    ...
    const [[asignado]] = await db.query(
      ...
      [docente_id, proyecto.curso_id]
    );
    if (!asignado) return res.status(403).json({ msg: 'No tienes asignado este curso' });

    // INSERT o UPDATE automático (por UNIQUE uq_nota_docente_proyecto)
    await db.query(
      `
      INSERT INTO notas (proyecto_id, docente_id, calificacion, observaciones)
      VALUES (?, ?, ?, ?)
      ...
      `,
      [proyecto_id, docente_id, calificacion, observaciones || null]
    );
...
exports.verNotasProyecto = async (req, res) => {
  try {
    ...
    // - docente: solo si está asignado al curso
    ...
    if (!['admin','secretaria','docente','estudiante'].includes(rol)) {
      return res.status(403).json({ msg: 'No autorizado' });
    }
