...
router.get('/excel', verifyToken, async (req, res) => {
  try {
    ...
    // HOJA: RESUMEN GENERAL (SIN NOTAS)
    ...
    for (const combo of combinaciones) {
      ...
      // Encabezados (SIN CALIF / NOTAS)
      const headerRow = ws.addRow(['Proyecto', 'Descripción', 'Estado', 'Tutor', 'Memoria', 'Archivo', 'Fecha']);
      ...
      proyectos.forEach((p, i) => {
        ...
        // Color memoria
        if (p.Memoria === 'Sí') row.getCell(5).font = { color: { argb: 'FF28A745' }, bold: true };
        else row.getCell(5).font = { color: { argb: 'FFDC3545' } };
      });

      // Totales (SIN PROMEDIOS / NOTAS)
      ws.addRow([]);
